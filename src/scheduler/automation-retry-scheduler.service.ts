...
export class AutomationRetrySchedulerService {
  ...
  async retryFailedAutomations(): Promise<void> {
    ...
    const retryable = failedLogs.filter((log) => ['drive-folder-provisioned', 'content-calendar-reminder'].includes(log.eventName));

    for (const log of retryable.slice(0, 5)) {
      try {
        await this.automationRetryService.retry(log.id);
      } catch (error) {
        this.logger.warn(`Scheduled retry failed for ${log.id}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
}
